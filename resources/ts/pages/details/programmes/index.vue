...
import type { MouvementRow, ProgrammeRow } from '@/types/details'
...
const { loading, error, items, stats, meta, fetch } = useProgrammesExplorer()
...
const heroStats = computed(() => {
  ...
  return [
    { label: 'Programmes', value: String(t.programmes_count) },
...
const kpiCards = computed(() => {
  ...
  if (!t) {
    return [
      { key: 'programmes', label: 'Programmes actifs', value: '—', accent: 'neutral' as const, icon: 'tabler-layout-grid' },
...
const selectedProgramme = computed(() =>
  stats.value?.programmes.find(p => p.code === programmeFilter.value) ?? null,
)
...
const typeMandatChart = computed(() => ({
  labels: stats.value?.par_type_mandat.map(t => t.label) ?? [],
  datasets: [{ data: stats.value?.par_type_mandat.map(t => t.montant) ?? [] }],
}))
...
function selectProgramme(prog: ProgrammeRow) {
  programmeFilter.value = programmeFilter.value === prog.code ? null : prog.code
...
onMounted(async () => {
  ...
  if (route.query.programme)
    programmeFilter.value = String(route.query.programme)
...
const headers = [
  ...
  { title: 'Chapitre', key: 'chapitre', width: '100px' },
...
<template>
  <div class="aice-page aice-explorer">
    ...
    >
      <VRow>
        <VCol
          v-for="prog in stats.programmes"
          :key="prog.code"
          cols="12"
          sm="6"
          md="4"
        ...
        >
          <VCard
            :class="['aice-programme-card', { 'aice-programme-card--active': programmeFilter === prog.code }]"
            elevation="0"
            @click="selectProgramme(prog)"
          >
            <VCardText class="pa-4">
              <div class="d-flex align-center justify-space-between mb-2">
                <span class="text-h6 font-weight-bold">{{ prog.code }}</span>
                <VChip
                  size="x-small"
                  :color="prog.taux_execution_pct >= 70 ? 'success' : prog.taux_execution_pct >= 40 ? 'warning' : 'error'"
                  variant="tonal"
                >
                  {{ prog.taux_execution_pct }} %
                </VChip>
              </div>
              ...
              <div class="d-flex justify-space-between text-body-2">
                <span>{{ prog.count }} mandats</span>
                <span class="font-weight-medium">{{ formatFcfa(prog.montant_depenses) }}</span>
              </div>
              <VProgressLinear
                :model-value="prog.taux_execution_pct"
                color="primary"
                height="4"
                rounded
                class="mt-3"
    ...
    <VAlert
      ...
      class="mb-4"
      closable
      @click:close="selectProgramme(selectedProgramme)"
    >
      Programme <strong>{{ selectedProgramme.code }}</strong> — {{ selectedProgramme.paye_count }} payés / {{ selectedProgramme.count }} mandats
    </VAlert>
    ...
    >
      ...
      >
        ...
        >
          <ChartWidget
            v-if="typeMandatChart.labels.length"
            type="bar"
            :labels="typeMandatChart.labels"
    ...
    <DataPanel
      :title="`Mandats du programme (${meta?.total ?? 0})`"
